            ...
            for node in tree.traverse():
                ...
                if self.split == "train":
                    ...
                    if "does not " in node.sent:
                        create_negative(premises, node.sent.replace("does not ", f"does not {random_word} "))
                    elif "do not " in node.sent:
                        create_negative(premises, node.sent.replace("do not ", f"do not {random_word} "))
                    elif "cannot " in node.sent:
                        create_negative(premises, node.sent.replace("cannot ", f"cannot {random_word} "))
                    elif "not " in node.sent:
                        create_negative(premises, node.sent.replace("not ", f"not {random_word} "))

                    if node.sent.startswith("i don't think "):
                        create_negative(
                            premises, node.sent.replace("i don't think ", f"i don't think {random_word} ")
                        )
                    else:
